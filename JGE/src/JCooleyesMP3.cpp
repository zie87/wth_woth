...
bool JCooleyesMP3::IsPlaying() { return mPlaying; }
...
bool JCooleyesMP3::Load(const char* filename) {
    if (!g_MP3DecoderOK) return false;
    ...
    if (fileSys->OpenFile(filename)) {
        ...
        if (mFileBuffer) {
            if (fileSys->ReadFile(mFileBuffer, mFileSize) != mFileSize) ret = false;
        } else
            ret = false;
...
void JCooleyesMP3::FeedAudioData(void* buf, unsigned int length /*, bool mixing*/) {
    if (mPlaying) {
        if (mSamplesPending > 0) {
            ...
            if ((int)length > mSamplesPending) length = mSamplesPending;
            ...
            for (i = 0; i < count; i++) {
                ...
                mDataPointer++;
                if (mDataPointer >= bufferMax) mDataPointer = 0;
            }
...
void JCooleyesMP3::Decode() {
    if (mAllMP3DataProcessed) return;
...
void JCooleyesMP3::Stop() { mPlaying = false; }

void JCooleyesMP3::Resume() {
    if (!mAllMP3DataProcessed) mPlaying = true;
}
