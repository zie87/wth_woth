...
int Ninja::canPay() {
    if (source->getObserver()->getCurrentGamePhase() != MTG_PHASE_COMBATBLOCKERS) return 0;
    return 1;
}
...
CounterCost::CounterCost(Counter* _counter, TargetChooser* _tc) : ExtraCost("Counters", _tc) {
    counter = _counter;
    hasCounters = 0;
}
