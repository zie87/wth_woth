...
bool JAnimator::Load(const char* scriptFile) {
    ...
    if (script) {
        ...
        const char* type[] = {"ANIMATION_TYPE_LOOPING", "ANIMATION_TYPE_ONCE_AND_STAY", "ANIMATION_TYPE_ONCE_AND_BACK",
...
void JAnimator::Pause() { mAnimating = false; }
...
JResourceManager* JAnimator::GetResourceManager() { return mResource; }
...
void JAnimatorFrame::AddObject(JAnimatorObject* obj) { mObjects.push_back(obj); }
...
void JAnimatorObject::SetPosition(float x, float y) {
    mX = x;
    mY = y;
}
