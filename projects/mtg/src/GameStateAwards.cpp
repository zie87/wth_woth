...
GameStateAwards::GameStateAwards(GameApp* parent) : GameState(parent, "trophies") {}
...
void GameStateAwards::Start() {
    ...
    aw = NEW WGuiAward(Options::AWARD_COLLECTOR, "Valuable Collection", "Collection valued over 10,000c.",
                       "Collection Info");
...
bool GameStateAwards::enterStats(int option) {
    ...
    if (setlist.size() > 0) {
        ...
        if (strong) {
            sprintf(buf, _("Most Powerful: %i (%s)").c_str(), strong->data->getPower(),
                    strong->data->getName().c_str());
            detailview->Add(NEW WGuiItem(buf, WGuiItem::NO_TRANSLATE));
        }
        if (tough) {
