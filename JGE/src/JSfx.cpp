...
JSample::JSample() { mSample = NULL; }

JSample::~JSample() {
    if (mSample) releaseWaveData(mSample);
}

unsigned long JSample::fileSize() { return mSample->fileSize; }
...
void JSoundSystem::InitSoundSystem() { audioInit(); }

void JSoundSystem::DestroySoundSystem() { audioDestroy(); }
...
JSample* JSoundSystem::LoadSample(const char* fileName) {
    ...
    if (sample) {
        ...
        if (!loadWaveData(sample->mSample, s, 1)) sample->mSample = NULL;
    }

    return sample;
}

void JSoundSystem::PauseMusic(JMusic* music) { StopMP3(); }

void JSoundSystem::PlayMusic(JMusic* music, bool looping) {
    if (music->mTrack) PlayMP3(music->mTrack, looping);
    JMP3* mp3 = JMP3::mInstance;
    if (mp3) mp3->setVolume((mMusicVolume * .01) * 0x8000);
}

void JSoundSystem::PlaySample(JSample* sample) { playWaveMem(sample->mSample, 0); }
...
void JSoundSystem::SetSfxVolume(int volume) { setPspVolume((volume * .01) * 0x8000); }

void JSoundSystem::StopMusic(JMusic* music) { StopMP3(); }

void JSoundSystem::ResumeMusic(JMusic* music) { ResumeMP3(music->mTrack); }
