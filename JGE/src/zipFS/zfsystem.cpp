...
filesystem::filesystem(const char* BasePath, const char* FileExt, bool DefaultFS)
    ...
    // The case insensitive state is mostly because some of the filesystems we support such as FAT32 are case
...
#ifdef USE_ZBUFFER_POOL
                zbuffer* buffer =
                    getValidBuffer(zipName, Filename, streamoff(DataPos), streamoff(FileInfo.m_CompSize));
...
#else
                ((izstream&)File)
                    .open(zipName.c_str(), streamoff(DataPos), streamoff(FileInfo.m_CompSize), FileInfo.m_CompMethod);
                if (File) {
#endif
                    File.m_FilePath = Filename;
                    File.m_FullFilePath = FullPath;
...
void filesystem::InsertZip(const char* Filename, const size_t PackID) {
    ...
    while ((NextHeader(File) == FILE) && (FileHdr.ReadHeader(File))) {
        ...
        if (FileHdr.m_FilenameSize != 0) {
            m_Files[Name] = file_info(PackID,                                  // Package ID
