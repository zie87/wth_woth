...
struct Vector3D {
    ...
    Vector3D operator*(const float scalar) const { return Vector3D(x * scalar, y * scalar, z * scalar); }

    // dot product
    float operator*(const Vector3D& rhs) const { return x * rhs.x + y * rhs.y + z * rhs.z; }
