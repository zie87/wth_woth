...
private:
    void init(string s, Spell* spell, MTGCardInstance* card) {
        ...
        if (!card->storedCard) card->storedCard = card->storedSourceCard;
        intValue = 0;
        bool halfup = false;
        bool halfdown = false;
        ...
        if (s == "prex") {
            ...
            std::string theType = s.substr(begins + 5);
            ...
            for (int i = 0; i < 2; i++) {
                ...
                for (int k = 0; k < 4; k++) {
                    ...
                    if (tc->targetsZone(zone, target)) intValue += zone->countByCanTarget(tc);
                }
            }
            SAFE_DELETE(tc);
            ...
            if (convertedType.size())
                ...
                if (convertedType.size())
                    ...
                    if (convertedType.size()) toughnessCheck = true;
                }
            }
            if (!convertedType.size()) return;
            bool high = false;
            int highest = 0;
            int lowest = 5000;
            if (convertedType[1].find("highest") != string::npos) high = true;

            std::string theType = convertedType[2];
            size_t zoned = theType.find(":");
            if (zoned == string::npos) {
                theType.append("|mybattlefield");
            } else {
                    ...
                    if (tc->targetsZone(zone, target)) {
                        for (unsigned int w = 0; w < zone->cards.size(); ++w) {
                            ...
                            if (tc->canTarget(cCard)) {
                                if (costCheck) check = cCard->getManaCost()->getConvertedCost();
                                if (powerCheck) check = cCard->power;
                                if (toughnessCheck) check = cCard->toughness;
            ...
            if (card->playerTarget) intValue = card->playerTarget->curses.size();
        } else if (s == "lifetotal") {
            ...
            if (checkagain > intValue) intValue = checkagain;
            if (card && card->thatmuch > intValue) intValue = card->thatmuch;
        ...
        if (intValue > 0) {
            ...
            if (halfdown) intValue = intValue / 2;
        }
        intValue *= multiplier;
    }
...
public:
    ...
    WParsedInt(string s, Spell* spell, MTGCardInstance* card) { init(s, spell, card); }

    WParsedInt(string s, MTGCardInstance* card) { init(s, NULL, card); }
...
public:
    ...
    TrCardAddedToZone(GameObserver* observer, int id, MTGCardInstance* source, TargetZoneChooser* toTcZone,
                      TargetChooser* toTcCard, TargetZoneChooser* fromTcZone = NULL, TargetChooser* fromTcCard = NULL,
                      bool once = false, bool sourceUntapped = false, bool isSuspended = false)
    ...
    int triggerOnEventImpl(WEvent* event) {
        ...
        if (!e) return 0;
        if (sourceUntapped && source->isTapped() == 1) return 0;
    ...
    TrCardAddedToZone* clone() const { return NEW TrCardAddedToZone(*this); }
};
...
public:
    ...
    TrCardTapped* clone() const { return NEW TrCardTapped(*this); }
};
...
public:
    ...
    TrCardTappedformana* clone() const { return NEW TrCardTappedformana(*this); }
};
...
public:
    ...
    TrCombatTrigger(GameObserver* observer, int id, MTGCardInstance* source, TargetChooser* tc,
                    TargetChooser* fromTc = NULL, bool once = false, bool limitOnceATurn = false,
    ...
    int triggerOnEventImpl(WEvent* event) {
        ...
        if (attacked && attackingTrigger && !attacked->card->didblocked) {
            if (!attacked->card->didattacked) return 0;
        ...
        if (attackedAlone && attackedAloneTrigger && !attackedAlone->card->didblocked) {
            if (!attackedAlone->card->didattacked) return 0;
        ...
        if (blocked && blockingTrigger && !blocked->card->didattacked) {
            if (!blocked->card->didblocked) return 0;
        ...
        if (notblocked && notBlockedTrigger && !notblocked->card->didblocked) {
            if (!notblocked->card->didattacked) return 0;
            if (notblocked->card->isBlocked()) return 0;
        ...
        if (attackblocked && attackBlockedTrigger && !attackblocked->card->didblocked) {
            if (!attackblocked->card->didattacked) return 0;
            if (!attackblocked->card->isBlocked()) return 0;
    ...
    ~TrCombatTrigger() { SAFE_DELETE(fromTc); }

    TrCombatTrigger* clone() const { return NEW TrCombatTrigger(*this); }
};
...
public:
    ...
    TrcardDrawn* clone() const { return NEW TrcardDrawn(*this); }
};
...
public:
    ...
    TrCardSacrificed* clone() const { return NEW TrCardSacrificed(*this); }
};
...
public:
    ...
    TrCardDiscarded* clone() const { return NEW TrCardDiscarded(*this); }
};
...
public:
    ...
    bool limitOnceATurn;
    int triggeredTurn;
    TrDamaged(GameObserver* observer, int id, MTGCardInstance* source, TargetChooser* tc, TargetChooser* fromTc = NULL,
              int type = 0, bool sourceUntapped = false, bool limitOnceATurn = false, bool once = false)
        : Trigger(observer, id, source, once, tc),
          fromTc(fromTc),
          type(type),
          sourceUntapped(sourceUntapped),
    ...
    ~TrDamaged() { SAFE_DELETE(fromTc); }

    TrDamaged* clone() const { return NEW TrDamaged(*this); }
};
...
public:
    ...
    TrLifeGained(GameObserver* observer, int id, MTGCardInstance* source, TargetChooser* tc,
                 TargetChooser* fromTc = NULL, int type = 0, bool sourceUntapped = false, bool once = false)
    ...
    ~TrLifeGained() { SAFE_DELETE(fromTc); }

    TrLifeGained* clone() const { return NEW TrLifeGained(*this); }
};
...
public:
    ...
    TrVampired(GameObserver* observer, int id, MTGCardInstance* source, TargetChooser* tc,
               TargetChooser* fromTc = NULL, bool once = false)
    ...
    int triggerOnEventImpl(WEvent* event) {
        ...
        if (!tc->canTarget(vamp->victem)) return 0;
        return 1;
    }

    ~TrVampired() { SAFE_DELETE(fromTc); }

    TrVampired* clone() const { return NEW TrVampired(*this); }
};
...
public:
    ...
    TrTargeted(GameObserver* observer, int id, MTGCardInstance* source, TargetChooser* tc,
               TargetChooser* fromTc = NULL, int type = 0, bool once = false)
    ...
    ~TrTargeted() { SAFE_DELETE(fromTc); }

    TrTargeted* clone() const { return NEW TrTargeted(*this); }
};
...
public:
    ...
    ATutorialMessage(GameObserver* observer, MTGCardInstance* source, std::string message, int limit = 1);
...
public:
    ...
    AACounter(GameObserver* observer, int id, MTGCardInstance* source, MTGCardInstance* target,
              std::string counterstring, const char* _name, int power, int toughness, int nb, int maxNb = 0,
              ManaCost* cost = NULL);
...
public:
    ...
    AARemoveAllCounter(GameObserver* observer, int id, MTGCardInstance* source, MTGCardInstance* target,
                       const char* _name, int power, int toughness, int nb, bool all, ManaCost* cost = NULL);
...
public:
    ...
    ANewAffinity(GameObserver* observer, int _id, MTGCardInstance* _source, std::string Tc = "",
                 std::string mana = "");
    void Update(float dt);
    int testDestroy();
...
public:
    ...
    int type;
    std::string Cond;
    IfThenAbility(GameObserver* observer, int _id, MTGAbility* delayedAbility = NULL,
                  MTGAbility* delayedElseAbility = NULL, MTGCardInstance* _source = NULL, Targetable* target = NULL,
                  int type = 1, std::string Cond = "");
    int resolve();
    const char* getMenuText();
...
public:
    ...
    MenuAbility(GameObserver* observer, int _id, Targetable* target, MTGCardInstance* _source, bool must = false,
                vector<MTGAbility*> abilities = vector<MTGAbility*>(), Player* who = NULL);
    void Update(float dt);
    int resolve();
    const char* getMenuText();
    int testDestroy();
...
public:
    ...
    GenericActivatedAbility(GameObserver* observer, std::string newName, std::string castRestriction, int _id,
                            MTGCardInstance* card, MTGAbility* a, ManaCost* _cost, std::string limit = "",
                            MTGAbility* sideEffects = NULL, std::string usesBeforeSideEffects = "",
...
public:
    AALibraryBottom(GameObserver* observer, int _id, MTGCardInstance* _source, MTGCardInstance* _target = NULL,
                    ManaCost* _cost = NULL);
    int resolve();
    const char* getMenuText();
...
public:
    AACopier(GameObserver* observer, int _id, MTGCardInstance* _source, MTGCardInstance* _target = NULL,
             ManaCost* _cost = NULL);
    int resolve();
    const char* getMenuText();
...
public:
    AAPhaseOut(GameObserver* observer, int _id, MTGCardInstance* _source, MTGCardInstance* _target = NULL,
               ManaCost* _cost = NULL);
    int resolve();
    const char* getMenuText();
...
public:
    ...
    AACloner(GameObserver* observer, int _id, MTGCardInstance* _source, MTGCardInstance* _target = NULL,
             ManaCost* _cost = NULL, int who = 0, std::string abilitiesStringList = "", std::string typeslist = "");
    int resolve();
    const char* getMenuText();
...
public:
    ...
    AAMover(GameObserver* observer, int _id, MTGCardInstance* _source, MTGCardInstance* _target, std::string dest,
            ManaCost* _cost = NULL);
...
public:
    std::string abilityTC;
    std::string fromZone;
    std::string toZone;
    AARandomMover(GameObserver* observer, int _id, MTGCardInstance* _source, MTGCardInstance* _target, std::string tcs,
                  std::string from, std::string to);
    MTGGameZone* destinationZone(Targetable* target = NULL, std::string zone = "");
    int resolve();
    const char* getMenuText();
...
public:
    ...
    std::string usesBeforeSideEffects;
    std::string tcString;

    GenericTargetAbility(GameObserver* observer, std::string newName, std::string castRestriction, int _id,
                         MTGCardInstance* _source, TargetChooser* _tc, MTGAbility* a, ManaCost* _cost = NULL,
                         std::string limit = "", MTGAbility* sideEffects = NULL,
                         std::string usesBeforeSideEffects = "", int restrictions = 0, MTGGameZone* dest = NULL,
                         std::string tcString = "");
    const char* getMenuText();
...
public:
    ...
    const char* getMenuText() { return "Ninjutsu"; }

    ANinja* clone() const { return NEW ANinja(*this); }
};
...
public:
    ...
    const char* getMenuText() { return "Remove From Combat"; }

    ACombatRemoval* clone() const { return NEW ACombatRemoval(*this); }
};
...
public:
    ...
    MaxPerTurnRestriction* existingRestriction;  // a pointer to the restriction that is being modified or that has
                                                 // been created (for restriction deletion purpose)
...
public:
    std::string life_s;
    AALifer(GameObserver* observer, int _id, MTGCardInstance* card, Targetable* _target, std::string life_s,
            ManaCost* _cost = NULL, int who = TargetChooser::UNSET);
    int resolve();
    const char* getMenuText();
    AALifer* clone() const;
    int getLife();
...
public:
    AAWinGame(GameObserver* observer, int _id, MTGCardInstance* card, Targetable* _target, ManaCost* _cost = NULL,
              int who = TargetChooser::UNSET);
    int resolve();
    const char* getMenuText();
...
public:
    ...
    ABasicAbilityModifier* clone() const { return NEW ABasicAbilityModifier(*this); }
};
...
public:
    ...
    int destroy() {
        MTGCardInstance* _target = (MTGCardInstance*)target;
        if (_target) _target->basicAbilities.set(ability, stateBeforeActivation);
        return 1;
    }
    ...
    AInstantBasicAbilityModifierUntilEOT* clone() const { return NEW AInstantBasicAbilityModifierUntilEOT(*this); }
};
...
public:
    ...
    const char* getMenuText() { return ability->getMenuText(); }
    ...
    ~ABasicAbilityAuraModifierUntilEOT() { SAFE_DELETE(ability); }
};
...
public:
    ...
    ASpellCastLife* clone() const { return NEW ASpellCastLife(*this); }

    ~ASpellCastLife() { SAFE_DELETE(cost); }
};
...
public:
    ...
    AUnBlocker* clone() const { return NEW AUnBlocker(*this); }
};
...
public:
    ...
    AProtectionFrom(GameObserver* observer, int id, MTGCardInstance* _source, MTGCardInstance* _target,
                    TargetChooser* fromTc, std::string tcstr)
    ...
    ~AProtectionFrom() { SAFE_DELETE(fromTc); }
};
...
public:
    ...
    ~ACantBeTargetFrom() { SAFE_DELETE(fromTc); }
};
// Can't be blocked by...
...
public:
    ...
    ~ACantBeBlockedBy() { SAFE_DELETE(fromTc); }
};
...
public:
    ...
    APowerToughnessModifier(GameObserver* observer, int id, MTGCardInstance* _source, MTGCardInstance* _target,
                            WParsedPT* wppt, std::string PT, bool nonstatic)
    ...
    ~APowerToughnessModifier() { delete (wppt); }
};
...
public:
    ...
    ~GenericInstantAbility() { SAFE_DELETE(ability); }
};
...
public:
    ...
    ~GenericAbilityMod() { SAFE_DELETE(ability); }
};
...
public:
    ...
    int resolve() {
        ...
        if (!_target) return 0;
        REDamagePrevention* re = NEW REDamagePrevention(this, NEW CardTargetChooser(game, _target, NULL),
                                                        NEW PlayerTargetChooser(game, 0, 1, source->controller()));
        current[re] = 1;
    ...
    ACircleOfProtection* clone() const { return NEW ACircleOfProtection(*this); }
};
...
public:
    ...
    AStandardRegenerate* clone() const { return NEW AStandardRegenerate(*this); }
};
...
public:
    ...
    void Update(float dt) {
        if (newPhase != currentPhase && newPhase == phase &&
            game->currentPlayer == ((MTGCardInstance*)target)->controller()) {
    ...
    ARegularLifeModifierAura* clone() const { return NEW ARegularLifeModifierAura(*this); }
};
...
public:
    ...
    int SorterFunction() {
        ...
        int size = 0;
        size = (int)cards.size();
        if (compareZone) findMatchingAmount();
    ...
    int canBeInList(MTGCardInstance* card) {
        if (card->isPhased || source->isPhased) return 0;
    ...
    int added(Player* p) { return 1; }
    int removed(MTGCardInstance* card) { return 1; }
    ///////////////////////////////////////////////////////
    ~AAsLongAs() { SAFE_DELETE(ability); }
...
public:
    ...
    ALord(GameObserver* observer, int _id, MTGCardInstance* card, TargetChooser* _tc, int _includeSelf, MTGAbility* a)
        ...
        tc->targetter = NULL;
        includeSelf = _includeSelf;
        if (ability->aType == MTGAbility::STANDARD_PREVENT) aType = MTGAbility::STANDARD_PREVENT;
    }

    // returns true if it is me who created ability a attached to Damageable d
    bool isParentOf(Damageable* d, MTGAbility* a) {
        if (abilities.find(d) != abilities.end()) return (abilities[d] == a);
        return false;
    }
    ...
    int added(MTGCardInstance* card) { return _added(card); }

    int added(Player* p) { return _added(p); }
    ...
    ~ALord() { SAFE_DELETE(ability); }

    const char* getMenuText() {
        // Special case for move
        if (AAMover* move = dynamic_cast<AAMover*>(ability)) return move->getMenuText(tc);
...
public:
        ...
        if (tc->canTarget(card) && card != tc->source) {
            ...
            if (tc->source->hasSubtype(Subtypes::TYPE_CREATURE)) {
                for (size_t myChild = 0; myChild < tc->source->parentCards.size(); ++myChild) {
                    if (tc->source->parentCards[myChild] == card) return 1;
                }
            }
        }
        return 0;
    ...
    int removed(MTGCardInstance* card) {
        if (skills.find(card) != skills.end()) {
            if (!game->removeObserver(skills[card])) skills[card]->destroy();
            skills.erase(card);
        }
        return 1;
    }
...
public:
    ...
    int unequip() {
        if (source->target) {
            ...
            for (unsigned int w = 0; w < source->target->childrenCards.size(); w++) {
                ...
                if (child == source) source->target->childrenCards.erase(source->target->childrenCards.begin() + w);
            }
        }
        source->target = NULL;
    ...
    const char* getMenuText() { return "Equip"; }
    ...
    AEquip* clone() const { return NEW AEquip(*this); }
};
...
public:
    ...
    int tokenId;
    std::string name;
    std::string sabilities;
    std::string starfound;
    ...
    ATokenCreator(GameObserver* observer, int _id, MTGCardInstance* _source, Targetable* _target, ManaCost* _cost,
                  int tokenId, std::string starfound, WParsedInt* multiplier = NULL, int who = 0,
    ...
    ATokenCreator(GameObserver* observer, int _id, MTGCardInstance* _source, Targetable* _target, ManaCost* _cost,
                  std::string sname, std::string stypes, int _power, int _toughness, std::string sabilities,
                  std::string starfound, WParsedInt* multiplier = NULL, int who = 0, bool aLivingWeapon = false,
                  std::string spt = "")
        : ActivatedAbility(observer, _id, _source, _cost, 0),
        ...
        if (sabilities.find("battleready") != string::npos) battleReady = true;
        ...
        while (s.size()) {
            size_t found = s.find(" ");
            if (found != string::npos) {
                std::string toCheck = s.substr(0, found);
    ...
    int resolve() {
        ...
        for (int i = 0; i < multiplier->getValue(); ++i) {
            ...
            if (sabilities.find("token(") == string::npos) {
                ...
                std::string words = sabilities.substr(0, endAbility);
                tokenText.append(words);
                std::string sourcename = ((MTGCardInstance*)source)->name;
    ...
    ~ATokenCreator() { SAFE_DELETE(multiplier); }
};
...
public:
    ...
    ATargetedAbilityCreator(GameObserver* observer, int _id, MTGCardInstance* _source, Targetable* _target,
                            ManaCost* _cost, std::string _name, std::string abilityToAdd, int who = 0)
    ...
    const char* getMenuText() {
        if (name.size()) return name.c_str();
        return "Ability";
    }
...
public:
    ...
    int removeFromGame() { return removeAbilityFromGame(); }

    int addToGame() { return MTGAbility::addToGame(); }

    void Update(float dt) { resolve(); }
...
public:
    ...
    AADamager(GameObserver* observer, int _id, MTGCardInstance* _source, Targetable* _target, std::string d,
              ManaCost* _cost = NULL, int who = TargetChooser::UNSET);
    int resolve();
    const char* getMenuText();
    int getDamage();
...
public:
    TADamager(GameObserver* observer, int id, MTGCardInstance* card, ManaCost* _cost, std::string d,
              TargetChooser* _tc = NULL)
    ...
    TADamager* clone() const { return NEW TADamager(*this); }
};
...
public:
    ...
    AAWhatsMax(GameObserver* observer, int id, MTGCardInstance* card, MTGCardInstance* source, ManaCost* _cost = NULL,
               int value = 0);
    int resolve();
...
public:
    vector<MTGAbility*> currentAbilities;
    std::string flipStats;
    AAFlip(GameObserver* observer, int id, MTGCardInstance* card, MTGCardInstance* _target, std::string flipStats);
    int resolve();
    int testDestroy();
    const char* getMenuText();
    AAFlip* clone() const;
...
public:
    ...
    AADynamic(GameObserver* observer, int id, MTGCardInstance* card, Damageable* _target, int type = 0, int effect = 0,
              int who = 0, int amountsource = 1, MTGAbility* storedAbility = NULL, ManaCost* _cost = NULL);
    int resolve();
...
public:
    ...
    const char* getMenuText() { return "Swap power and toughness"; }
    ASwapPT* clone() const { return NEW ASwapPT(*this); }
};
...
public:
    ...
    ALifeZoneLink(GameObserver* observer, int _id, MTGCardInstance* card, int _phase, int _condition, int _life = -1,
                  int _controller = 0, MTGGameZone* _zone = NULL)
    ...
    ALifeZoneLink* clone() const { return NEW ALifeZoneLink(*this); }
};
...
public:
    ...
    AStrongLandLinkCreature* clone() const { return NEW AStrongLandLinkCreature(*this); }
};
...
public:
    ...
    AControlStealAura* clone() const { return NEW AControlStealAura(*this); }
};
// bloodthirst ability------------------------------------------
...
public:
    ...
    ABloodThirst* clone() const { return NEW ABloodThirst(*this); }
...
public:
    ...
    bool newpowerfound;
    int oldpower;
    std::string newtoughness;
    bool newtoughnessfound;
    int oldtoughness;
    std::map<Damageable*, std::vector<MTGAbility*> > newAbilities;
    std::vector<std::string> newAbilitiesList;
    bool newAbilityFound;
    bool aForever;

    ATransformer(GameObserver* observer, int id, MTGCardInstance* source, MTGCardInstance* target, std::string stypes,
                 std::string sabilities, std::string newpower, bool newpowerfound, std::string newtoughness,
                 bool newtoughnessfound, vector<string> newAbilitiesList, bool newAbilityFound = false,
                 bool aForever = false);
    int addToGame();
    int reapplyCountersBonus(MTGCardInstance* rtarget = NULL, bool powerapplied = false,
...
public:
    ...
    ATransformerInstant(GameObserver* observer, int id, MTGCardInstance* source, MTGCardInstance* target,
                        std::string types = "", std::string abilities = "", std::string newpower = "",
                        bool newpowerfound = false, std::string newtoughness = "", bool newtoughnessfound = false,
                        vector<string> newAbilitiesList = vector<string>(), bool newAbilityFound = false,
                        bool aForever = false);
    int resolve();
    const char* getMenuText();
...
public:
    ...
    PTInstant(GameObserver* observer, int id, MTGCardInstance* source, MTGCardInstance* target, WParsedPT* wppt,
              std::string s = "", bool nonstatic = false);
    int resolve();
    const char* getMenuText();
...
public:
    ...
    const char* getMenuText() { return "Exalted"; }

    AExalted* clone() const { return NEW AExalted(*this); }
};
...
public:
    ...
    APreventDamageTypes(GameObserver* observer, int id, MTGCardInstance* source, std::string to, std::string from,
                        int type = 0);
    int addToGame();
    int destroy();
...
public:
    ...
    int type;
    APreventDamageTypesUEOT(GameObserver* observer, int id, MTGCardInstance* source, std::string to, std::string from,
                            int type = 0);
    int resolve();
    int destroy();
    const char* getMenuText();
...
public:
    ...
    AVanishing(GameObserver* observer, int _id, MTGCardInstance* card, ManaCost* _cost, int restrictions = 0,
               int amount = 0, std::string counterName = "");
    void Update(float dt);
    int resolve();
    const char* getMenuText();
...
public:
    ...
    APhaseAction(GameObserver* observer, int _id, MTGCardInstance* card, MTGCardInstance* target, std::string sAbility,
                 int restrictions = 0, int _phase = MTG_PHASE_UPKEEP, bool forcedestroy = false, bool next = true,
                 bool myturn = true, bool opponentturn = true, bool once = false);
    void Update(float dt);
    int resolve();
    const char* getMenuText();
...
public:
    ...
    APhaseActionGeneric(GameObserver* observer, int _id, MTGCardInstance* card, MTGCardInstance* target,
                        std::string sAbility, int restrictions = 0, int _phase = MTG_PHASE_UPKEEP,
...
public:
    ...
    ABlink(GameObserver* observer, int _id, MTGCardInstance* card, MTGCardInstance* _target, bool blinkueot = false,
           bool blinkForSource = false, bool blinkhand = false, MTGAbility* stored = NULL);
    void Update(float dt);
    void resolveBlink();
    int resolve();
    const char* getMenuText();
...
public:
    ...
    AAladdinsLamp* clone() const { return NEW AAladdinsLamp(*this); }
};
...
public:
    int counters;
    ManaCost cost;
    AArmageddonClock(GameObserver* observer, int id, MTGCardInstance* _source) : MTGAbility(observer, id, _source) {
        counters = 0;
    ...
    AArmageddonClock* clone() const { return NEW AArmageddonClock(*this); }
};
...
public:
    int canprevent;
    ManaCost cost;
    AConservator(GameObserver* observer, int _id, MTGCardInstance* _source) : MTGAbility(observer, _id, _source) {
        canprevent = 0;
        std::vector<int8_t> _cost;
        _cost.push_back(Constants::MTG_COLOR_ARTIFACT);
        _cost.push_back(2);
    ...
    void Update(float dt) { alterDamage(); }
    ...
    AConservator* clone() const { return NEW AConservator(*this); }
};
...
public:
    ...
    AFarmstead* clone() const { return NEW AFarmstead(*this); }
};
...
public:
    ...
    AKjeldoranFrostbeast* clone() const { return NEW AKjeldoranFrostbeast(*this); }
};
...
public:
    ...
    AAnimateDead* clone() const { return NEW AAnimateDead(*this); }
};
...
public:
    ...
    AErgRaiders(GameObserver* observer, int _id, MTGCardInstance* _source) : MTGAbility(observer, _id, _source) {
        attackedThisTurn = 1;
    }
    ...
    AErgRaiders* clone() const { return NEW AErgRaiders(*this); }
};
...
public:
    ...
    AFastbond(GameObserver* observer, int _id, MTGCardInstance* card) : TriggeredAbility(observer, _id, card) {
    ...
    int trigger() {
        int landsPlayedThisTurnUpdated =
    ...
    int destroy() {
        PlayRestrictions* restrictions = source->controller()->game->playRestrictions;
        if (restrictions->getMaxPerTurnRestrictionByTargetChooser(counter)) return 1;
...
public:
    ...
    AJandorsRing* clone() const { return NEW AJandorsRing(*this); }
};
...
public:
    ...
    AKudzu* clone() const { return NEW AKudzu(*this); }
};
...
public:
    ...
    APowerLeak* clone() const { return NEW APowerLeak(*this); }
};
...
public:
    ...
    ASacrifice* clone() const { return NEW ASacrifice(*this); }
};
...
public:
    ...
    AEarthbind* clone() const { return NEW AEarthbind(*this); }
};
...
public:
    ...
    AFireball* clone() const { return NEW AFireball(*this); }
};
...
public:
    ...
    AIslandSanctuary(GameObserver* observer, int _id, MTGCardInstance* _source) : MTGAbility(observer, _id, _source) {
        initThisTurn = 0;
    }
    ...
    int isReactingToClick(MTGCardInstance* card, ManaCost* mana = NULL) {
        if (card == source && game->currentPlayer == card->controller()) {
            ...
            if (draw && draw->aType == MTGAbility::STANDARD_DRAW) return 1;
        }
        return 0;
    }
    ...
    AIslandSanctuary* clone() const { return NEW AIslandSanctuary(*this); }
};
...
public:
    ...
    int triggerOnEvent(WEvent* event) {
        if (!who) {
            ...
            if (targetingString.find("controller") != string::npos) who = source->controller();
            if (targetingString.find("opponent") != string::npos) who = source->controller()->opponent();
            if (targetingString.find("targetcontroller") != string::npos)
                who = source->target ? source->target->controller() : source->controller();
            if (targetingString.find("owner") != string::npos) who = source->owner;
        }
    ...
    void addTheEffect(int i) {
        ...
        if (turnSteps != turnAfterChange) applied = true;
        return;
    }
    ...
    const char* getMenuText() { return "phase alter"; }

    APhaseAlter* clone() const { return NEW APhaseAlter(*this); }
};
...
public:
    MTGCardInstance* opponents[20];
    int nbOpponents;
    ABasilik(GameObserver* observer, int _id, MTGCardInstance* _source) : MTGAbility(observer, _id, _source) {
        nbOpponents = 0;
    }
    ...
    ABasilik* clone() const { return NEW ABasilik(*this); }
};
...
public:
    ...
    AADepleter(GameObserver* observer, int _id, MTGCardInstance* card, Targetable* _target, std::string nbcardsStr,
               ManaCost* _cost = NULL, int who = TargetChooser::UNSET);
    int resolve();
    const char* getMenuText();
...
public:
    std::string nbTurnStr;

    AAModTurn(GameObserver* observer, int _id, MTGCardInstance* card, Targetable* _target, std::string nbTurnStr,
              ManaCost* _cost = NULL, int who = TargetChooser::UNSET);
    int resolve();
    const char* getMenuText();
...
public:
    AAShuffle(GameObserver* observer, int _id, MTGCardInstance* card, Targetable* _target, ManaCost* _cost = NULL,
              int who = TargetChooser::UNSET);
    int resolve();
    const char* getMenuText();
...
public:
    ...
    AARemoveMana(GameObserver* observer, int _id, MTGCardInstance* card, Targetable* _target, std::string ManaDesc,
                 int who = TargetChooser::UNSET);
    int resolve();
    const char* getMenuText();
...
public:
    ...
    AARandomDiscarder(GameObserver* observer, int _id, MTGCardInstance* card, Targetable* _target,
                      std::string nbcardsStr, ManaCost* _cost = NULL, int who = TargetChooser::UNSET);
    int resolve();
    const char* getMenuText();
...
public:
    ...
    ARampageAbility* clone() const { return NEW ARampageAbility(*this); }
};
...
public:
    MTGCardInstance* opponents[20];
    int nbOpponents;
    AFlankerAbility(GameObserver* observer, int _id, MTGCardInstance* _source) : MTGAbility(observer, _id, _source) {
        nbOpponents = 0;
    }
    int receiveEvent(WEvent* event) {
        if (dynamic_cast<WEventBlockersChosen*>(event)) {
    ...
    AFlankerAbility* clone() const { return NEW AFlankerAbility(*this); }
};

// Bushido ability
class ABushidoAbility : public MTGAbility {
public:
    std::string PowerToughnessModifier;

    ABushidoAbility(GameObserver* observer, int _id, MTGCardInstance* _source, std::string _PowerToughnessModifier)
        : MTGAbility(observer, _id, _source) {
    ...
    int receiveEvent(WEvent* event) {
        if (dynamic_cast<WEventBlockersChosen*>(event)) {
            ...
            PTInstant* a = NEW PTInstant(game, this->GetId(), source, source,
                                         NEW WParsedPT(PowerToughnessModifier, NULL, source));
    ...
    ABushidoAbility* clone() const { return NEW ABushidoAbility(*this); }
};
...
public:
    ...
    int receiveEvent(WEvent* event) {
        if (event->type == WEvent::DAMAGE) {
            ...
            Damage* d = e->damage;
            if (combatonly && e->damage->typeOfDamage != DAMAGE_COMBAT) return 0;
    ...
    ASpiritLinkAbility* clone() const { return NEW ASpiritLinkAbility(*this); }
};
...
public:
    ...
    int destroy() {
        MTGCardInstance* _target = (MTGCardInstance*)target;
        if (TheftController && TheftController->game->inPlay->hasCard(
                                   _target)) {  // if the target is still in game -> spell was destroyed
    ...
    AInstantControlSteal* clone() const { return NEW AInstantControlSteal(*this); }
};
...
public:
    ...
    AASetTypeChosen(GameObserver* observer, int id, MTGCardInstance* source, MTGCardInstance* target, int _type = 0,
                    std::string menu = "error", std::string toAdd = "");
    int resolve();
    const char* getMenuText();
...
public:
    ...
    GenericChooseTypeColor(GameObserver* observer, int id, MTGCardInstance* source, Targetable* target,
                           std::string toAdd = "", bool chooseColor = false, bool nonwall = false,
                           ManaCost* cost = NULL);
    int resolve();
    const char* getMenuText();
...
public:
    int side;
    std::string abilityToAlter;
    std::string abilityWin;
    std::string abilityLose;
    MTGAbility* abilityAltered;
...
public:
    ...
    bool chooseColor;
    AASetCoin* setCoin;
    GenericFlipACoin(GameObserver* observer, int id, MTGCardInstance* source, Targetable* target,
                     std::string toAdd = "", ManaCost* cost = NULL);
    int resolve();
    const char* getMenuText();
...
void PopulateColorIndexVector(std::list<int>& colors, const std::string& colorsString, char delimiter = ',');
void PopulateAbilityIndexVector(std::list<int>& abilities, const std::string& abilitiesString, char delimiter = ',');
void PopulateSubtypesIndexVector(std::list<int>& types, const std::string& subtypesString, char delimiter = ' ');
