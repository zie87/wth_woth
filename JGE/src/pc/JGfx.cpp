...
JTexture::~JTexture() {
    checkGlError();
    if (mTexId != (GLuint)-1) glDeleteTextures(1, &mTexId);
    checkGlError();
...
void JRenderer::RenderQuad(JQuad* quad, float xo, float yo, float angle, float xScale, float yScale) {
    ...
    glVertexAttribPointer(prog2_positionLoc, 3, GL_FLOAT, GL_FALSE, 5 * sizeof(GLfloat), vVertices);

    // Load the texture coordinate
    glVertexAttribPointer(prog2_texCoordLoc, 2, GL_FLOAT, GL_FALSE, 5 * sizeof(GLfloat), &vVertices[3]);

    // Load the colors
    glVertexAttribPointer(prog2_colorLoc, 4, GL_UNSIGNED_BYTE, GL_TRUE, 4 * sizeof(GLubyte), colorCoords);
...
void JRenderer::FillRect(float x, float y, float width, float height, PIXEL_TYPE color) {
...
#if (defined GL_ES_VERSION_2_0) || (defined GL_VERSION_2_0)
    GLfloat vVertices[] = {
        x, y + height, 0.0f, x, y, 0.0f, x + width, y + height, 0.0f, x + width, y, 0.0f,
    };

    GLubyte colors[] = {
        col.r, col.g, col.b, col.a, col.r, col.g, col.b, col.a, col.r, col.g, col.b, col.a, col.r, col.g, col.b, col.a,
    };
    ...
    // Load the vertex position
    glVertexAttribPointer(prog1_positionLoc, 3, GL_FLOAT, GL_FALSE, 3 * sizeof(GLfloat), vVertices);
    // Load the color
    glVertexAttribPointer(prog1_colorLoc, 4, GL_UNSIGNED_BYTE, GL_TRUE, 4 * sizeof(GLubyte), colors);
    ...
    GLfloat vVertices[] = {
        x, y + height, x, y, x + width, y + height, x + width, y,
    };
...
#if (defined GL_ES_VERSION_2_0) || (defined GL_VERSION_2_0)
    GLfloat vVertices[] = {
        x, y, 0.0f, x, y + height, 0.0f, x + width, y + height, 0.0f, x + width, y, 0.0f,
    };
...
void JRenderer::FillRect(float x, float y, float width, float height, PIXEL_TYPE* colors) {
    JColor col[4];
    for (int i = 0; i < 4; i++) col[i].color = colors[i];
...
void JRenderer::FillRect(float x, float y, float width, float height, JColor* colors) {
    ...
    GLubyte cols[] = {
        colors[0].r, colors[0].g, colors[0].b, colors[0].a, colors[2].r, colors[2].g, colors[2].b, colors[2].a,
        colors[1].r, colors[1].g, colors[1].b, colors[1].a, colors[3].r, colors[3].g, colors[3].b, colors[3].a,
    };
    ...
    glVertexAttribPointer(prog1_positionLoc, 3, GL_FLOAT, GL_FALSE, 3 * sizeof(GLfloat), vVertices);
    // Load the color
    glVertexAttribPointer(prog1_colorLoc, 4, GL_UNSIGNED_BYTE, GL_TRUE, 4 * sizeof(GLubyte), cols);
...
void JRenderer::DrawLine(float x1, float y1, float x2, float y2, PIXEL_TYPE color) {
...
#if (defined GL_ES_VERSION_2_0) || (defined GL_VERSION_2_0)
    GLfloat vVertices[] = {
        x1, SCREEN_HEIGHT_F - y1, 0.0f, x2, SCREEN_HEIGHT_F - y2, 0.0f,
    };

    GLubyte cols[] = {
        col.r, col.g, col.b, col.a, col.r, col.g, col.b, col.a,
    };
    ...
    if (color != 0xFFFFFFFF) glColor4ub(col.r, col.g, col.b, col.a);

    glBegin(GL_LINES);
    glVertex2f(x1, SCREEN_HEIGHT_F - y1);
    glVertex2f(x2, SCREEN_HEIGHT_F - y2);
    glEnd();
    if (color != 0xFFFFFFFF) glColor4ub(255, 255, 255, 255);
    glEnable(GL_TEXTURE_2D);
#endif  //#if (!defined GL_ES_VERSION_2_0) && (!defined GL_VERSION_2_0)
    checkGlError();
}
...
void JRenderer::PlotArray(float* x, float* y, int count, PIXEL_TYPE color) {
    ...
    glColor4ub(col.r, col.g, col.b, col.a);
    glBegin(GL_POINTS);
    for (int i = 0; i < count; i++) glVertex2f(x[i], SCREEN_HEIGHT_F - y[i]);
    glEnd();
    glColor4ub(255, 255, 255, 255);
#else
    // FIXME, not used
...
void JRenderer::Enable2D() {
    checkGlError();
    if (mCurrentRenderMode == MODE_2D) return;
...
void JRenderer::LoadIdentity() {  // NOT USED
                                  // glLoadIdentity();
}

void JRenderer::Translate(float x, float y, float z) {  // NOT USED
                                                        // glTranslatef(x, y, z);
}

void JRenderer::RotateX(float angle) {  // NOT USED
                                        // glRotatef(angle*RAD2DEG, 1.0f, 0.0f, 0.0f);
}

void JRenderer::RotateY(float angle) {  // NOT USED
                                        // glRotatef(angle*RAD2DEG, 0.0f, 1.0f, 0.0f);
}

void JRenderer::RotateZ(float angle) {  // NOT USED
                                        // glRotatef(angle*RAD2DEG, 0.0f, 0.0f, 1.0f);
}

void JRenderer::PushMatrix() {  // NOT USED
                                // glPushMatrix();
}

void JRenderer::PopMatrix() {  // NOT USED
                               // glPopMatrix();
}
...
void JRenderer::RenderTriangles(JTexture* texture, Vertex3D* vertices, int start, int count) {
    ...
    // Load the vertex position
    glVertexAttribPointer(prog2_positionLoc, 3, GL_FLOAT, GL_FALSE, 5 * sizeof(GLfloat), &vertices[index].x);
    // Load the texture coordinate
    glVertexAttribPointer(prog2_texCoordLoc, 2, GL_FLOAT, GL_FALSE, 5 * sizeof(GLfloat), &vertices[index].u);
    // Load the colors
    glVertexAttribPointer(prog2_colorLoc, 4, GL_UNSIGNED_BYTE, GL_TRUE, 4 * sizeof(GLubyte), colorCoords);
