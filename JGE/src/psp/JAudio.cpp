...
char loadWaveData(WAVDATA* p_wav, char* fileName, char memLoad)  // WAVE加载, memLoad-是否加载至内磥E
{
    ...
    if (!fileSystem->OpenFile(fileName)) return 0;
...
void audioOutCallback(int channel, void* buf, unsigned int length)  // 各通道回祦E
{
    ...
    if (!currentWav[channel].fullName[0]) {
        if (!p_currentWav[channel]) return;
        ...
        if (!p_wav->fullName[0])
            ...
            if (p_wav->bytePosition == 0 && p_wav->delayTime) sceKernelDelayThread(p_wav->delayTime);
        }
    } else {
    ...
    if (p_wav->flag & PW_PAUSE) return;
    int sizeStep = p_wav->sizeStep;
    int nSample = p_wav->nSample;
    if (p_wav->flag & PW_FAST) sizeStep = p_wav->sizeStep + (p_wav->sizeStep >> 1);
    char* tmpBuff;
    char byteRead[4096];
    int actualSize = 0;
    if (p_wav->fd >= 0) {
        ...
        for (j = 0; j < nSample; j++) {
            if (p_wav->bytePerSample == 2) {
                if (p_wav->channelCount == 2) {
                    srcPos = (i << 2) / nSample;
                    if (p_wav->flag & PW_FAST) srcPos = srcPos + (srcPos >> 1);
...
{
    stopWaveMem(channel);
    if (currentWav[channel].fullName[0]) releaseWaveData(&currentWav[channel]);
    if (!loadWaveData(&currentWav[channel], fullName, 0)) return 0;
...
void stopWaveMem(WAVDATA* pWav) {
    for (int i = 0; i < NUMBER_WAV_CHANNELS; ++i) {
        if (p_currentWav[i] == pWav) stopWaveMem(i);
    }
}
...
//////////////////////////////////////////////////////////////////////////
void ResumeMP3(JMP3* mp3) { mp3->play(); }
