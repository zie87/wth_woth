    ...
    while (found && (!maxDecks || nbDecks <= maxDecks)) {
        ...
        if (meta) {
            ...
            if (unlocked) {
                ...
                // updateMetaDataList in DeckManager.cpp performs some weird magic, swapping data between its cache and
                // the "retList" from this function Bottom line, we need to guarantee retList contains exactly the same
                // items as (or updated versions of) the items in DeckManager Cache In other words, any meta data that
                // didn't make it to retList in this function must be erased from the DeckManager cache
