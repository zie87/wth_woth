...
void RulesState::parsePlayerState(int playerId, string s) {
    if (playerData[playerId].player->parseLine(s)) return;
                ...
                if (a->oneShot) {
                            ...
                            GAME_TYPE_STORY)  // stupid protections to keep this out of mimor and other game modes.
                    {
                        ...
                        if (difficultyRating == EASY) {
                            ...
                            hand->OptimizedHand(p, handsize, 1, 0, 2);  // give the Ai deck a tiny boost by giving it 1
                                                                        // land and 2 spells under 3 manacost.
                        } else {
...
void Rules::initPlayers(GameObserver* g) {
    for (int i = 0; i < 2; i++) {
        ...
        if (p && g->getPlayersNumber() < 2) g->players.push_back(p);
...
void Rules::initGame(GameObserver* g) {
    ...
    if (g->currentPlayer->playMode != Player::MODE_TEST_SUITE && g->mRules->gamemode != GAME_TYPE_STORY) {
        ...
        if (OptionWhosFirst::WHO_O == options[Options::FIRSTPLAYER].number) initState.player = 1;
    }
...
int Rules::load(string _filename) {
    ...
    std::string contents;
    if (!JFileSystem::GetInstance()->readIntoString(c_filename, contents)) return 0;
