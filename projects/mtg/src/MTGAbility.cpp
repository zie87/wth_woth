...
int AbilityFactory::parseCastRestrictions(MTGCardInstance* card, Player* player, string restrictions) {
    ...
    for (unsigned int i = 0; i < restriction.size(); i++) {
        ...
        case MTGAbility::AS_SORCERY:
            if (player != observer->currentPlayer) return 0;
            if (cPhase != MTG_PHASE_FIRSTMAIN && cPhase != MTG_PHASE_SECONDMAIN) return 0;
            break;
        }
        if (checkPhaseBased >= MTGAbility::MY_BEFORE_BEGIN && checkPhaseBased <= MTGAbility::MY_AFTER_EOT) {
            if (player != observer->currentPlayer) return 0;
            if (cPhase != checkPhaseBased - MTGAbility::MY_BEFORE_BEGIN + MTG_PHASE_BEFORE_BEGIN) return 0;
        }
        if (checkPhaseBased >= MTGAbility::OPPONENT_BEFORE_BEGIN &&
            checkPhaseBased <= MTGAbility::OPPONENT_AFTER_EOT) {
            if (player == observer->currentPlayer) return 0;
            if (cPhase != checkPhaseBased - MTGAbility::OPPONENT_BEFORE_BEGIN + MTG_PHASE_BEFORE_BEGIN) return 0;
        }
        if (checkPhaseBased >= MTGAbility::BEFORE_BEGIN && checkPhaseBased <= MTGAbility::AFTER_EOT) {
            if (cPhase != checkPhaseBased - MTGAbility::BEFORE_BEGIN + MTG_PHASE_BEFORE_BEGIN) return 0;
        }
        ...
        if (seenRelated == string::npos) seenRelated = restriction[i].find("thisturn(");
        ...
        if (typeRelated != string::npos || seenRelated != string::npos || compRelated != string::npos) {
            ...
            if (comparasion.size() != 3) return 0;  // it was incorrectly coded, user should proofread card code.
            ...
            for (unsigned int i = 0; i < comparasion.size(); i++) {
                ...
                if (check == string::npos) check = comparasion[i].find("lastturn(");
                if (check == string::npos) check = comparasion[i].find("thisturn(");
                if (check == string::npos) check = comparasion[i].find("compare(");
            ...
            if (firstAmount < secondAmount && !less && !more && !equal) return 0;
            if (equal && firstAmount != secondAmount) return 0;
            if (less && firstAmount >= secondAmount) return 0;
            if (more && firstAmount <= secondAmount) return 0;
        }
        ...
        if (check != string::npos) {
            ...
            if (observer->turn < Turn - 1) return 0;
        }
        check = restriction[i].find("casted a spell");
        if (check != string::npos) {
            if (player->game->stack->seenThisTurn("*", Constants::CAST_ALL) < 1) return 0;
        }
        ...
        if (check != string::npos) {
            ...
            if (tc.find("|mystack") != string::npos) {
                if (player->game->stack->seenThisTurn(tc, Constants::CAST_ALL) < 1) return 0;
        ...
        if (check != string::npos) {
            ...
            if (!isMorbid) return 0;
        }
        check = restriction[i].find("one of a kind");
        if (check != string::npos) {
            if (player->game->inPlay->hasName(card->name)) return 0;
        }
        ...
        if (check != string::npos) {
            if (cPhase < MTG_PHASE_COMBATBEGIN || cPhase > MTG_PHASE_COMBATEND) return 0;
        }
        check = restriction[i].find("control snow land");
        if (check != string::npos) {
            if (!player->game->inPlay->hasPrimaryType("snow", "land")) return 0;
        }
...
int AbilityFactory::parseRestriction(string s) {
    ...
    if (s.find("opponentturnonly") != string::npos) return ActivatedAbility::OPPONENT_TURN_ONLY;
    if (s.find("assorcery") != string::npos) return ActivatedAbility::AS_SORCERY;

    string types[] = {"my", "opponent", ""};
    int starts[] = {ActivatedAbility::MY_BEFORE_BEGIN, ActivatedAbility::OPPONENT_BEFORE_BEGIN,
...
TargetChooser* AbilityFactory::parseSimpleTC(const std::string& s, const std::string& _starter, MTGCardInstance* card,
    ...
    if (tc && forceNoTarget) tc->targetter = NULL;
...
TriggeredAbility* AbilityFactory::parseTrigger(string s, string magicText, int id, Spell* spell, MTGCardInstance* card,
    ...
    if (found != string::npos) {
        ...
        TriggeredAbility* mover =
            NEW TrCardAddedToZone(observer, id, card, (TargetZoneChooser*)toTc, toTcCard, (TargetZoneChooser*)fromTc,
    ...
    // Card card is drawn
    if (TargetChooser* tc = parseSimpleTC(s, "drawn", card)) return NEW TrcardDrawn(observer, id, card, tc, once);
    ...
    if (s.find(" targetedplayer") != string::npos) who = -3;
...
MTGAbility* AbilityFactory::getCoreAbility(MTGAbility* a) {
    if (AForeach* fea = dynamic_cast<AForeach*>(a)) return getCoreAbility(fea->ability);

    if (AAsLongAs* aea = dynamic_cast<AAsLongAs*>(a)) return getCoreAbility(aea->ability);

    if (GenericTargetAbility* gta = dynamic_cast<GenericTargetAbility*>(a)) return getCoreAbility(gta->ability);

    if (GenericActivatedAbility* gaa = dynamic_cast<GenericActivatedAbility*>(a)) return getCoreAbility(gaa->ability);
    ...
    if (MenuAbility* ma = dynamic_cast<MenuAbility*>(a)) return getCoreAbility(ma->abilities[0]);
...
MTGAbility* AbilityFactory::parseMagicLine(string s, int id, Spell* spell, MTGCardInstance* card, bool activated,
    ...
    if (splitTrigger.size()) {
        ...
        if (splitTrigger[1].find("restriction{") != string::npos)  // using other/cast restrictions for abilities.
        {
            ...
            if (splitRest.size()) trigger->castRestriction = splitRest[1];
        }
    ...
    if (delimiter != string::npos && firstNonSpace != string::npos && sWithoutTc[firstNonSpace] == '{') {
        ...
        if (doTap || cost) {
            ...
            if (amp) {
                amp->setCost(cost);
                if (cost) cost->setExtraCostsAction(a, card);
                amp->oneShot = 0;
                amp->tap = doTap;
                amp->limit = limit;
            ...
            return NEW GenericActivatedAbility(observer, newName, castRestriction, id, card, a, cost, limit,
                                               sideEffect, usesBeforeSideEffect, restrictions, dest);
        }
        SAFE_DELETE(cost);
    }
    ...
    for (size_t i = 0; i < sizeof(mayMust) / sizeof(mayMust[0]); ++i) {
        if (sWithoutTc.find(mayKeywords[i]) == 0) {
            ...
            if (mayCost) mainAbility->optionalCost = mayCost;
            return mainAbility;
        }
    }
    ...
    if (found != string::npos) forcedalive = 1;
    ...
    if (found != string::npos && found < lord) {
        ...
        if (end != string::npos) {
            ...
            found = s.find("while ");
            if (found != string::npos) oneShot = false;
    ...
    if (found != string::npos) {
        ...
        if (end != string::npos) {
            ...
            MTGAbility* result = NULL;
            bool oneShot = false;
            if (activated || i == 4 || a->oneShot) oneShot = true;
            if (card->hasType(Subtypes::TYPE_SORCERY) || card->hasType(Subtypes::TYPE_INSTANT)) oneShot = true;
            found = s.find("while ");
            if (found != string::npos) oneShot = false;
            found = s.find(" oneshot");
            if (found != string::npos) oneShot = true;
            Damageable* _target = NULL;
            if (spell) _target = spell->getNextDamageableTarget();
            if (!_target) _target = target;
    ...
    if (splitDynamic.size()) {
        ...
        if (!sAbility.empty()) stored = parseMagicLine(sAbility, id, spell, card);
    ...
    if (s.find(" targetedplayer") != string::npos) who = TargetChooser::TARGETED_PLAYER;
    ...
    if (s.find("ability$") != string::npos) {
        if (storedAbilityString.size()) {
            ATargetedAbilityCreator* abl =
                NEW ATargetedAbilityCreator(observer, id, card, target, NULL, newName, storedAbilityString, who);
            abl->oneShot = 1;
            storedString.clear();
            return abl;
        }
    ...
    if (splitToken.size()) {
        ...
        if (tokenId) {
            ...
            if (!safetycard)  // Error, card not foudn in DB
                return NEW ATokenCreator(observer, id, card, target, NULL, "ID NOT FOUND", "ERROR ID", 0, 0, "", "",
                                         NULL, 0);

            ATokenCreator* tok =
                NEW ATokenCreator(observer, id, card, target, NULL, tokenId, starfound, multiplier, who);
            tok->oneShot = 1;
            return tok;
        }
        ...
        ATokenCreator* tok =
            NEW ATokenCreator(observer, id, card, target, NULL, sname, stypes, power + value, toughness + value,
                              sabilities, starfound, multiplier, who, aLivingWeapon, spt);
        tok->oneShot = 1;
        if (aLivingWeapon) tok->forceDestroy = 1;
        return tok;
    }
    ...
    if (splitMove.size()) {
        ...
        if (card->hasType(Subtypes::TYPE_AURA)) target = card;
    ...
    if (splitRandomMove.size()) {
        ...
        if (!splitfrom.size() || !splitto.size()) return NULL;
    ...
    found = s.find("ueot");
    if (found != string::npos) forceUEOT = true;
    found = s.find("oneshot");
    if (found != string::npos) oneShot = true;
    found = s.find("forever");
    if (found != string::npos) forceForever = true;
    // Prevent Damage
    ...
    for (size_t i = 0; i < sizeof(preventDamageTypes) / sizeof(preventDamageTypes[0]); ++i) {
        ...
        if (found != string::npos) {
            ...
            if (splitFrom.size()) from = splitFrom[1];
            ...
            if (preventDamageForceOneShot[i]) ab->oneShot = 1;
    ...
    if (splitPhaseAlter.size()) {
        ...
        if (splitPhaseAlter2.size() < 3) return NULL;
        string after = "";
    ...
    if (wppt->ok) {
        if (s.find("nonstatic") != string::npos) nonstatic = true;
        if (!activated) {
            if (card->hasType(Subtypes::TYPE_INSTANT) || card->hasType(Subtypes::TYPE_SORCERY) || forceUEOT) {
    ...
    if (found != string::npos) {
        ...
        a->oneShot = 1;
        if (newName.size()) ((AManaProducer*)a)->menutext = newName;
        return a;
    }
    ...
    if (splitNewAffinity.size()) {
        ...
        if (!manaString.size()) return NULL;
...
MTGAbility* AbilityFactory::parseUpkeepAbility(string s, MTGCardInstance* card, Spell* spell, int restrictions,
                                               int id) {
    bool Cumulative = false;
    size_t cumulative = s.find("cumulativeupcost");
    if (cumulative != string::npos) Cumulative = true;
    size_t start = s.find("[");
    size_t end = s.find("]", start);
...
MTGAbility* AbilityFactory::parsePhaseActionAbility(string s, MTGCardInstance* card, Spell* spell,
    ...
    if (spell) _target = spell->getNextCardTarget();
    if (!_target) _target = target;
...
MTGAbility* AbilityFactory::parseChooseActionAbility(string s, MTGCardInstance* card, Spell* spell,
                                                     MTGCardInstance* target, int restrictions, int id) {
...
int AbilityFactory::abilityEfficiency(MTGAbility* a, Player* p, int mode, TargetChooser* tc, Targetable* target) {
    ...
    if (ATeach* abi = dynamic_cast<ATeach*>(a)) return abilityEfficiency(abi->ability, p, mode, tc);
    ...
    if (dynamic_cast<AEquip*>(a)) return BAKA_EFFECT_GOOD;
    ...
    if (dynamic_cast<ACounterTracker*>(a)) return BAKA_EFFECT_GOOD;
    ...
    if (AALifer* abi = dynamic_cast<AALifer*>(a)) return abi->getLife() > 0 ? BAKA_EFFECT_GOOD : BAKA_EFFECT_BAD;
    ...
    if (APowerToughnessModifier* abi = dynamic_cast<APowerToughnessModifier*>(a))
        return (abi->wppt->power.getValue() >= 0 && abi->wppt->toughness.getValue() >= 0) ? BAKA_EFFECT_GOOD
                                                                                          : BAKA_EFFECT_BAD;
    if (PTInstant* abi = dynamic_cast<PTInstant*>(a)) return abilityEfficiency(abi->ability, p, mode, tc);
...
int AbilityFactory::magicText(int id, Spell* spell, MTGCardInstance* card, int mode, TargetChooser* tc,
    ...
    for (size_t i = 0; i < v.size(); ++i) {
        ...
        if (!a->oneShot) {
            ...
            if (dynamic_cast<AManaProducer*>(core)) a->canBeInterrupted = false;
        }

        bool moreThanOneTarget = spell && spell->tc && spell->tc->getNbTargets() > 1;

        if (moreThanOneTarget) a->target = spell->getNextTarget();
...
void AbilityFactory::addAbilities(int _id, Spell* spell) {
    ...
    case 1103:  // Crystal Rod
    {
        ...
        cost.push_back(1);
        ASpellCastLife* ability =
            NEW ASpellCastLife(observer, _id, card, Constants::MTG_COLOR_BLUE, NEW ManaCost(cost, 1), 1);
        observer->addObserver(ability);
        break;
    }
    case 1152:  // Deathlace
    ...
    case 1113:  // Iron Star
    {
        ...
        cost.push_back(1);
        ASpellCastLife* ability =
            NEW ASpellCastLife(observer, _id, card, Constants::MTG_COLOR_RED, NEW ManaCost(cost, 1), 1);
        observer->addObserver(ability);
        break;
    }
    ...
    case 1114:  // Ivory cup
    {
        ...
        cost.push_back(1);
        ASpellCastLife* ability =
            NEW ASpellCastLife(observer, _id, card, Constants::MTG_COLOR_WHITE, NEW ManaCost(cost, 1), 1);
        observer->addObserver(ability);
        break;
    }
    ...
    case 1140:  // Throne of Bone
    {
        ...
        cost.push_back(1);
        ASpellCastLife* ability =
            NEW ASpellCastLife(observer, _id, card, Constants::MTG_COLOR_BLACK, NEW ManaCost(cost, 1), 1);
        observer->addObserver(ability);
        break;
    }
    ...
    case 1142:  // Wooden Sphere
    {
        ...
        cost.push_back(1);
        ASpellCastLife* ability =
            NEW ASpellCastLife(observer, _id, card, Constants::MTG_COLOR_GREEN, NEW ManaCost(cost, 1), 1);
        observer->addObserver(ability);
        break;
    }
...
//          -> manaredux(green,-2)
MTGAbility* AbilityFactory::getManaReduxAbility(string s, int id, Spell* spell, MTGCardInstance* card,
...
MTGAbility::MTGAbility(GameObserver* observer, int id, MTGCardInstance* card) : ActionElement(id) {
    game = observer;
    source = card;
    target = card;
...
int MTGAbility::testDestroy() {
    ...
    if (source->graveEffects && game->isInGrave(source)) return 0;
    if (source->exileEffects && game->isInExile(source)) return 0;
    if (this->forcedAlive == 1) return 0;
    if (!game->isInPlay(source)) return 1;
    if (target && !dynamic_cast<Player*>(target) && !game->isInPlay((MTGCardInstance*)target)) return 1;
    return 0;
}
...
Player* MTGAbility::getPlayerFromTarget(Targetable* target) {
    ...
    if (MTGCardInstance* cTarget = dynamic_cast<MTGCardInstance*>(target)) return cTarget->controller();

    if (Player* cPlayer = dynamic_cast<Player*>(target)) return cPlayer;
...
Player* MTGAbility::getPlayerFromDamageable(Damageable* target) {
    ...
    if (target->type_as_damageable == DAMAGEABLE_MTGCARDINSTANCE) return ((MTGCardInstance*)target)->controller();
...
ActivatedAbility::ActivatedAbility(GameObserver* observer, int id, MTGCardInstance* card, ManaCost* _cost,
                                   int restrictions, string limit, MTGAbility* sideEffect,
...
int ActivatedAbility::isReactingToClick(MTGCardInstance* card, ManaCost* mana) {
    ...
    if (restrictions >= MY_BEFORE_BEGIN && restrictions <= MY_AFTER_EOT) {
        if (player != game->currentPlayer) return 0;
        if (cPhase != restrictions - MY_BEFORE_BEGIN + MTG_PHASE_BEFORE_BEGIN) return 0;
    }

    if (restrictions >= OPPONENT_BEFORE_BEGIN && restrictions <= OPPONENT_AFTER_EOT) {
        if (player == game->currentPlayer) return 0;
        if (cPhase != restrictions - OPPONENT_BEFORE_BEGIN + MTG_PHASE_BEFORE_BEGIN) return 0;
    }

    if (restrictions >= BEFORE_BEGIN && restrictions <= AFTER_EOT) {
        if (cPhase != restrictions - BEFORE_BEGIN + MTG_PHASE_BEFORE_BEGIN) return 0;
    }
    limitPerTurn = 0;
    if (limit.size()) {
    ...
    if (card == source && source->controller() == player &&
        ...
        if (source->has(Constants::NOMANA) && (amp || femp)) return 0;
...
TargetAbility::TargetAbility(GameObserver* observer, int id, MTGCardInstance* card, TargetChooser* _tc,
                             ManaCost* _cost, int _playerturnonly, string castRestriction)
    : ActivatedAbility(observer, id, card, _cost, _playerturnonly, "", NULL, "", castRestriction),
      NestedAbility(NULL)  // Todo fix this mess, why do we have to pass "", NULL, "" here before cast restrictions?
{
    tc = _tc;
}
...
int TargetAbility::resolve() {
    ...
    if (t && ability) {
        ...
        if (dynamic_cast<MTGCardInstance*>(t) && ((MTGCardInstance*)t)->isPhased) return 0;
...
int Trigger::triggerOnEvent(WEvent* event) {
    if (!mActiveTrigger) return 0;

    // Abilities don't work if the card is phased
    if (source->isPhased) return 0;

    if (!triggerOnEventImpl(event)) return 0;

    if (mOnce && mActiveTrigger) mActiveTrigger = false;

    if (castRestriction.size()) {
        if (!source) return 1;  // can't check these restrictions without a source aka:in a rule.txt
        AbilityFactory af(game);
...
int InstantAbility::testDestroy() {
    ...
    if (newPhase != currentPhase && newPhase == MTG_PHASE_AFTER_EOT) return 1;
...
int TriggerAtPhase::trigger() {
    ...
    if (lifelost) {
        ...
        if (lifeloss < lifeamount) return 0;
    }
    if (sourceUntapped && source->isTapped() == 1) return 0;
    if (sourceTap && !source->isTapped()) return 0;
    ...
    if (currentPhase != newPhase && newPhase == phaseId) {
        ...
        switch (who) {
        case 1:
            if (game->currentPlayer == source->controller()) result = 1;
            break;
        case -1:
            if (game->currentPlayer != source->controller()) result = 1;
            break;
        case -2:
            if (source->target) {
                if (game->currentPlayer == source->target->controller()) result = 1;
            } else {
                if (game->currentPlayer == source->controller()) result = 1;
            }
            break;
        case -3:
            if (source->playerTarget) {
                if (game->currentPlayer == source->playerTarget) result = 1;
            }
            break;
        default:
            result = 1;
        ...
        if (castRestriction.size()) {
            if (!source) result = 1;  // can't check these restrictions without a source aka:in a rule.txt
            AbilityFactory af(game);
            int checkCond = af.parseCastRestrictions(source, source->controller(), castRestriction);
            if (!checkCond) result = 0;
        }
    }
    if (once && activeTrigger) activeTrigger = false;
...
Targetable* GenericTriggeredAbility::getTriggerTarget(WEvent* e, MTGAbility* a) {
    ...
    if (ttc) return e->getTarget(ttc->triggerTarget);
...
int AManaProducer::reactToClick(MTGCardInstance* _card) {
    ...
    if (!ActivatedAbility::isReactingToClick(_card)) return 0;
