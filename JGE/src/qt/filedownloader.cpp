...

{
    ...
    if (m_WebCtrl.networkAccessible()) {
        ...
        requestHash(QUrl("https://storage.googleapis.com/google-code-archive-downloads/v2/code.google.com/wagic/" +
                         m_targetFile));
    }
...
void FileDownloader::setDownloadUrl(QUrl url) {
    if ((!url.isEmpty())) {
        ...
        connect(m_downloadReply, SIGNAL(downloadProgress(qint64, qint64)), SLOT(downloadProgress(qint64, qint64)));
...
void FileDownloader::computeRemoteHash() {
    if (m_hashReply->error() != QNetworkReply::NoError) {
        ...
        if (m_localHash != m_remoteHash) { /* We download the real file */
            m_state = DOWNLOADING_FILE;
            setDownloadUrl(
                QUrl("https://storage.googleapis.com/google-code-archive-downloads/v2/code.google.com/wagic/" +
                     m_targetFile));
        } else {
            m_state = DOWNLOADED;
        }
