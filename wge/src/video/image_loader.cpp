...
namespace wge
{
  namespace video
  {
    ...
    texture_data::texture_data(wge::size_t w, wge::size_t h, wge::size_t tw, wge::size_t th, wge::byte_t* buf, wge::size_t channels) noexcept
...
static int stbi_cb_eof(void* user) {
    ...
    texture_data image_loader::load_image(std::istream& stream)
    {
      ...
      int channels = 0;
       ...
       stbi_io_callbacks cb = {
          stbi_cb_read, stbi_cb_skip, stbi_cb_eof
      };
    ...
    wge::byte_t*
    image_loader::convert_pixel_buffer(const wge::byte_t* const buffer, const int width, const int height, const int channels, int& new_width, int& new_height)
    {
