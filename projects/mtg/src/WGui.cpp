...
// WGuiItem
void WGuiItem::Entering(JButton key) { mFocus = true; }
...
void WGuiHeader::Render() {
    WFont* mFont = WResourceManager::Instance()->GetWFont(Fonts::OPTION_FONT);
    // save the current scaling factor.  We don't want the lists to change font size these lists should stay the same
    // no matter what
...
void WGuiMenu::subBack(WGuiBase* item) {
    ...
    if (split && split->left->Visible() && split->right->Visible()) {
        if (split->left)
            subBack(
                split
                    ->left);  // renderer->FillRoundRect(split->left->getX()-2,split->getY()-2,split->left->getWidth()-6,split->getHeight(),2,split->left->getColor(WGuiColor::BACK));
        if (split->right)
            subBack(
                split
                    ->right);  // renderer->FillRoundRect(split->right->getX()-2,split->getY()-2,split->right->getWidth(),split->getHeight(),2,split->right->getColor(WGuiColor::BACK));
    } else
        renderer->FillRoundRect(item->getX(), item->getY(), item->getWidth() - 4, item->getHeight() - 2, 2,
                                item->getColor(WGuiColor::BACK));
}
...
void WGuiList::Render() {
    ...
    if (!items.size() && failMsg != "") {
        WFont* mFont = WResourceManager::Instance()->GetWFont(Fonts::OPTION_FONT);
        // save the current scaling factor.  We don't want the lists to change font size these lists should stay the
        // same no matter what
...
bool WGuiList::CheckUserInput(JButton key) {
    ...
    if ((key == JGE_BTN_OK) &&
        mEngine->GetLeftClickCoordinates(
            i, j)) {  // a dude clicked somwhere, we're gonna select the closest object from where he clicked
        int n = currentItem;
...
WDecoConfirm::~WDecoConfirm() { SAFE_DELETE(confirmMenu); }
...
bool WGuiSplit::CheckUserInput(JButton key) {
    ...
    if (hasFocus()) {
        ...
        if (key == JGE_BTN_NONE &&
            JGE::GetInstance()->GetLeftClickCoordinates(
                i, j)) {  // a dude clicked somwhere, we're gonna select the closest object from where he clicked
            unsigned int distanceLeft, distanceRight;

            distanceLeft = static_cast<unsigned int>((left->getY() - j) * (left->getY() - j) +
                                                     (left->getX() - i) * (left->getX() - i));
            distanceRight = static_cast<unsigned int>((right->getY() - j) * (right->getY() - j) +
                                                      (right->getX() - i) * (right->getX() - i));
...
bool WGuiMenu::CheckUserInput(JButton key) {
    ...
    if (mEngine->GetLeftClickCoordinates(
            i, j)) {  // a dude clicked somwhere, we're gonna select the closest object from where he clicked
        int n = currentItem;
        unsigned int distance2;
        unsigned int minDistance2 = -1;
...
void WGuiMenu::syncMove() {
    ...
    while (i < 0 && sync->prev()) i = currentItem - sync->getPos();
    while (i > 0 && sync->next()) i = currentItem - sync->getPos();
}
...
bool WGuiTabMenu::CheckUserInput(JButton key) {
    ...
    if (mEngine->GetLeftClickCoordinates(i, j)) {
        if (j <= 25) {  // a dude clicked in the tab title bar, let's compute which tab from i
            float offset = x;
            WFont* mFont = WResourceManager::Instance()->GetWFont(Fonts::OPTION_FONT);
            mFont->SetScale(0.8f);
            for (vector<WGuiBase*>::iterator it = items.begin(); it != items.end(); it++) {
...
void WGuiAward::Overlay() {
    ...
    if (s.size()) {
        ...
        if (fH < 16) fH = 18;
        JQuadPtr button =
            WResourceManager::Instance()->RetrieveQuad("iconspsp.png", (float)4 * 32, 0, 32, 32, "", RETRIEVE_NORMAL);
...
void WGuiAward::Underlay() {
    ...
    if (!trophy && id >= Options::SET_UNLOCKS) {
        trophy = WResourceManager::Instance()->RetrieveTempQuad(
            "trophy_set.png");  // TODO FIXME: Should look in set dir too.
    }
...
void WGuiAward::Render() {
    ...
    myX += fM;
    JRenderer::GetInstance()->FillRoundRect(x - fM / 2, y - fM, getWidth() - fM, fH - fM, fM,
                                            getColor(WGuiColor::BACK_TAB));
...
WGuiAward::WGuiAward(string _id, string name, string _text, string _details) : WGuiItem(name) {
    id = 0;
    textId = _id;
    text = _text;
    height = 60;
...
float WGuiImage::getHeight() {
    ...
    if (imgH == 0) {
        if (source && q.get()) return MAX(height, q->mHeight + (2 * margin));
    }
...
WGuiCardImage::WGuiCardImage(WDataSource* wds, bool _thumb) : WGuiImage(wds) { bThumb = _thumb; };
...
void WGuiCardImage::Render() {
    ...
    if (!source || (c = source->getCard(mOffset.getPos())) == NULL) {  // No card, use card back.
        ...
        if (bThumb) {
            ...
            if (!q) q = WResourceManager::Instance()->GetQuad(kGenericCardID);
#endif
        } else
...
#if defined WIN32 || defined LINUX
                if (!q) q = source->getImage(mOffset.getPos());
#endif
            }
            if (!q.get()) {
...
void WGuiFilters::recolorFilter(int color) { recolorTo = color; }
...
void WGuiFilters::setSrc(WSrcCards* wsc) { source = wsc; }
...
void WGuiFilterItem::updateValue() {
    ...
    case STATE_UNSET:
        ...
        SAFE_DELETE(mParent->subMenu);
        mParent->subMenu =
            NEW SimpleMenu(JGE::GetInstance(), -1234, this, Fonts::MENU_FONT, 20, 20, "Filter By...", 6);
    ...
    case STATE_CHOOSE_TYPE:
        ...
        if (filterType == FILTER_TYPE) {
            ...
            for (int i = Subtypes::LAST_TYPE + 1;; i++) {
                ...
                if (s == "Nothing") {  // dont add "nothing" to the search filters.
                } else {
                    stlist.push_back(s);
                }
            }
...
static const JButton btnToCheck[] = {JGE_BTN_MENU, JGE_BTN_CTRL, JGE_BTN_RIGHT, JGE_BTN_LEFT,
...
void WGuiKeyBinder::ButtonPressed(int controllerId, int controlId) {
    if (2 == controlId) switch (controllerId) {
        case 0:
...
bool WGuiKeyBinder::yieldFocus() { return true; }
