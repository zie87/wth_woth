...
void GameStateShop::Start() {
    ...
    for (int i = 0; i < 8; ++i) {
        ...
        stream << "iconspsp" << i;
        pspIcons[i] = WResourceManager::Instance()->RetrieveQuad("iconspsp.png", (float)i * 32, 0, 32, 32,
        ...
        if (filterMenu) {
            ...
            if (filterMenu->isFinished()) {
                if (needLoad) {
                    ...
                    srcCards->addFilter(NEW WCFilterNOT(NEW WCFilterSet(MTGSets::INTERNAL_SET)));
                    if (!srcCards->Size()) {
                        srcCards->clearFilters();  // Repetition of check at end of filterMenu->Finish(), for the token
                                                   // removal
                        srcCards->addFilter(NEW WCFilterNOT(NEW WCFilterRarity("T")));
                        srcCards->addFilter(NEW WCFilterNOT(NEW WCFilterSet(MTGSets::INTERNAL_SET)));
                    }
...
void GameStateShop::Render() {
    ...
    r->RenderQuad(pspIcons[6].get(), SCREEN_WIDTH - len - 0.5 - 10, SCREEN_HEIGHT - 8, 0, kPspIconScaleFactor,
                  kPspIconScaleFactor);
...
string ShopBooster::getName() {
    ...
    if (altSet == mainSet) altSet = nullptr;
    if (altSet)
...
void ShopBooster::randomCustom(MTGPacks* packlist) {
    ...
    if (pack && !pack->isUnlocked()) pack = nullptr;
...
void ShopBooster::randomStandard() {
    ...
    if (altSet == mainSet) altSet = nullptr;  // Prevent "10E & 10E Booster"
...
bool ShopBooster::unitTest() {
    ...
    // Uncommon in that case also see http://code.google.com/p/wagic/issues/detail?id=644
    if (!s && (r != 1 || u != 3)) {
