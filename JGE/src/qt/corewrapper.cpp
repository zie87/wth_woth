...
#ifdef Q_WS_MAEMO_5
    dBusInterface = new QDBusInterface(MCE_SERVICE, MCE_REQUEST_PATH, MCE_REQUEST_IF, dBusConnection);

    // Handle screen state on / off
    dBusConnection.connect(MCE_SERVICE, MCE_SIGNAL_PATH, MCE_SIGNAL_IF, MCE_DISPLAY_SIG, this,
...
WagicCore::~WagicCore() {
#ifdef Q_WS_MAEMO_5
    if (dBusInterface) delete dBusInterface;
#endif  // Q_WS_MAEMO_5
...
void WagicCore::pixelInput(int x, int y) {
    if (m_engine) m_engine->LeftClicked(x, y);
}
...
void WagicCore::timerEvent(QTimerEvent*) {
    ...
    if (!m_engine) return;
    if (m_engine->IsDone()) QApplication::instance()->quit();
...
void WagicCore::resizeGL(int width, int height) {
    ...
    glViewport(m_viewPort.left(), m_viewPort.top(), m_viewPort.right() - m_viewPort.left(),
    ...
    #if (defined GL_VERSION_ES_CM_1_1 || defined GL_OES_VERSION_1_1)
    glOrthof(0.0f, (float)(m_viewPort.right() - m_viewPort.left()) - 1.0f, 0.0f,
             (float)(m_viewPort.bottom() - m_viewPort.top()) - 1.0f, -1.0f, 1.0f);
    #else
    gluOrtho2D(0.0f, (float)(m_viewPort.right() - m_viewPort.left()) - 1.0f, 0.0f,
               (float)(m_viewPort.bottom() - m_viewPort.top()) - 1.0f);
    #endif
...
void WagicCore::mousePressEvent(QMouseEvent* event) {
    if (event->button() == Qt::LeftButton) {
        ...
        if (lastPos.y() >= m_viewPort.top() && lastPos.y() <= m_viewPort.bottom() &&
            lastPos.x() <= m_viewPort.right() && lastPos.x() >= m_viewPort.left()) {
            m_engine->LeftClicked(((lastPos.x() - m_viewPort.left()) * SCREEN_WIDTH) / actualWidth,
        ...
        event->accept();
    } else if (event->button() == Qt::RightButton) { /* next phase please */
        m_engine->HoldKey_NoRepeat(JGE_BTN_PREV);
        event->accept();
    } else if (event->button() == Qt::MidButton) { /* interrupt please */
        m_engine->HoldKey_NoRepeat(JGE_BTN_SEC);
        event->accept();
    } else {
...
void WagicCore::mouseReleaseEvent(QMouseEvent* event) {
    if (event->button() == Qt::LeftButton) {
        ...
        if (lastPos.y() >= m_viewPort.top() && lastPos.y() <= m_viewPort.bottom() &&
            ...
            } else if (g_startTimer.elapsed() - mLastFingerDownTime >= kSwipeEventMinDuration) {  // Let's swipe
...
void WagicCore::mouseMoveEvent(QMouseEvent* event) {
    ...
    if (lastPos.y() >= m_viewPort.top() && lastPos.y() <= m_viewPort.bottom() && lastPos.x() <= m_viewPort.right() &&
...
void WagicCore::showEvent(QShowEvent* event) { setActive(true); }

void WagicCore::hideEvent(QHideEvent* event) { setActive(false); }
