    ...
    if ((target)->preventable >= 1) {
        ...
        for (int k = preventing; k > 0; k--) {
            ...
            for (int i = damage; i >= 1; i--) {
                ...
                damage -= 1;
                break;  // does the redux of damage 1 time, breaks the loop to deincrement preventing and start the
                        // loop over.
            }
        }
    }
...
DamageStack::DamageStack(GameObserver* observer) : GuiLayer(observer), Interruptible(observer) {
    currentState = -1;
    type = ACTION_DAMAGES;
}
