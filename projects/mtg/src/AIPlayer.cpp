...
int AIPlayer::Act(float dt) {
    if (observer->currentPlayer == this) observer->userRequestNextGamePhase();
    return 1;
}
...
int AIPlayer::clickSingleTarget(TargetChooser* tc, vector<Targetable*>& potentialTargets,
...
int AIPlayer::getTotalAIDecks() {
    if (totalAIDecks == -1) totalAIDecks = countTotalDecks(0, 0, 1);
    return totalAIDecks;
}
...
int AIPlayer::countTotalDecks(int lower, int higher, int current) {
    ...
    if (JFileSystem::GetInstance()->FileExists(buffer)) {
        if (higher == current + 1) return current;
