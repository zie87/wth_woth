...
void JTTFont::SetAngle(float angle) { mAngle = angle; }

bool JTTFont::SetSize(int size) {
    if (!mFace) return false;

    if (mSize == size) return true;
...
int JTTFont::PreCacheChar(u16 ch, u16 cachedCode) {
    for (int i = 0; i < mMaxCharCount && mCachedCode[i]; i++) {
        if (mCachedCode[i] == cachedCode) return i;
    }
    ...
    if (!mAntialias) renderFlag |= FT_LOAD_TARGET_MONO;
    ...
    if (mCurr >= mMaxCharCount) mCurr = 0;
...
void JTTFont::PreCacheString(const u16* text) { RenderString(text, 0, 0, false); }

void JTTFont::PreCacheString(const char* text) { RenderString(text, 0, 0, false); }

void JTTFont::PreCacheString(const u8* text) { RenderString(text, 0, 0, false); }

int JTTFont::GetWidth(const u16* text) { return RenderString(text, 0, 0, false); }

int JTTFont::GetWidth(const char* text) { return RenderString(text, 0, 0, false); }

int JTTFont::GetWidth(const u8* text) { return RenderString(text, 0, 0, false); }
...
bool JTTFont::PreCacheASCII() {
    int count = 127 - 32 + 1;
    if (count > mMaxCharCount) count = mMaxCharCount;
...
void JTTFont::SetAntialias(bool flag) { mAntialias = flag; }
