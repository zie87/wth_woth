...
#define TEXTURES_CACHE_MINSIZE \
    2000000  // Minimum size of the cache on the PSP. The program should complain if the cache ever gets smaller than
             // this
#define OPERATIONAL_SIZE \
    5000000  // Size required by Wagic for operational stuff. 3MB is not enough. The cache will usually try to take
...
public:
    ...
    void Resize(unsigned long size,
                int items);  // Sets new limits, then enforces them. Lock safe, so not a "hard limit".
...
protected:
    ...
    */
    ...
    bool RemoveItem(cacheItem* item,
                    bool force = true);  // Removes an item, deleting it. if(force), ignores locks / permanent
    bool UnlinkCache(cacheItem* item);   // Removes an item from our cache, does not delete it. Use with care.
    bool Delete(cacheItem* item);  // SAFE_DELETE and garbage collect. If maxCached == 0, nullify first. (This means
                                   // you have to free that cacheActual later!)
