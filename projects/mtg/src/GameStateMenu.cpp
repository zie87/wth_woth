        ...
        if (mGuiController) {
            ...
            for (size_t i = 0; i < items.size(); ++i) {
                ...
                mGuiController->Add(NEW MenuItem(item->mActionId, mFont, item->mDisplayName, startX + (i * space),
                                                 40 + SCREEN_HEIGHT / 2, mIcons[iconId].get(),
                                                 mIcons[iconId + 1].get(), item->mParticleFile.c_str(),
...
void GameStateMenu::Render() {
    ...
    if ((currentState & MENU_STATE_MAJOR) == MENU_STATE_MAJOR_LANG) {
        ...
        PIXEL_TYPE colors[] = {
            static_cast<PIXEL_TYPE>(ARGB(255, 3, 3, 0)),
...
void GameStateMenu::ButtonPressed(int controllerId, int controlId) {
    ...
    default:
        ...
        default:  // Game modes
            ...
            mParent->gameType = (mParent->rules->gamemode);  // TODO can we get rid of gameType in the long run, since
                                                             // it is also stored in the rules object ?
            subMenuController->Close();
            currentState = MENU_STATE_MAJOR_DUEL | MENU_STATE_MINOR_SUBMENU_CLOSING;
            break;
        }
...
std::ostream& GameStateMenu::toString(std::ostream& out) const {
    return out << "GameStateMenu ::: scroller : " << scroller << " ; scrollerSet : " << scrollerSet
               ...
               << " ; mIcons : " << mIcons << " ; bgTexture : " << bgTexture << " ; mBg : " << mBg
               << " ; mCreditsYPos : " << mCreditsYPos << " ; currentState : " << currentState
               << " ; mVolume : " << mVolume << " ; nbcardsStr : " << nbcardsStr
               << " ; mCurrentSetName : " << mCurrentSetName << " ; mCurrentSetFileName : " << mCurrentSetFileName
               << " ; mReadConf : " << mReadConf << " ; timeIndex : " << timeIndex;
}
