...
bool JParticleEffect::Load(const char* filename) {
    ...
    const char* lifeValues[] = {"speed", "size",         "rotation",         "alpha",  "red", "green",
                                "blue",  "radial_accel", "tangential_accel", "gravity"};

    const char* typeNames[] = {"POINT", "AREA", "HORIZONTAL", "VERTICAL", "CIRCLE"};

    const char* modeNames[] = {"REPEAT", "ONCE", "NTIMES", "CONTINUOUS"};
    ...
    if (effect) {
        ...
        for (emitter = effect->FirstChild("emitter"); emitter && mEmitterCount < MAX_EMITTER;
             emitter = emitter->NextSibling()) {
            ...
            for (param = emitter->FirstChild(); param; param = param->NextSibling()) {
                ...
                if (strcmp(element->Attribute("name"), "settings") == 0) {
                    ...
                    // TIXML_SUCCESS)
                    // 					{
                    // 						mParticleEmitters[mEmitterCount]->mQuadIndex =
                    // int_value;
                    //
                    // 					}
...
void JParticleEffect::SetParticleSystem(JParticleSystem* particleSys) { mParticleSystem = particleSys; }
