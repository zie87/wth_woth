...
void JSoundSystem::PlayMusic(JMusic* music, bool looping) {
    ...
    NSLog(@"Playing music file %@",
          [NSString stringWithCString:music->filename.c_str() encoding:NSUTF8StringEncoding]);
...
JSample* JSoundSystem::LoadSample(const char* fileName) {
    ...
    if (sample) {
        NSArray* components =
            [[NSString stringWithCString:fileName encoding:NSUTF8StringEncoding] componentsSeparatedByString:@"."];
        string fullpath = JFileSystem::GetInstance()->GetResourceFile(fileName);
        ...
        if ([key isEqualToString:@""]) return sample;
