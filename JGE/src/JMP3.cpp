...
JMP3::JMP3() : m_volume(PSP_AUDIO_VOLUME_MAX), m_samplesPlayed(0), m_paused(true) {}
...
bool JMP3::fillBuffers() {
    ...
    if (sceIoLseek32(m_fileHandle, pos, SEEK_SET) < 0) {
        ...
        if (sceIoLseek32(m_fileHandle, 0, SEEK_END) != m_fileSize || sceIoLseek32(m_fileHandle, pos, SEEK_SET) < 0) {
            sceIoClose(m_fileHandle);
            m_fileHandle = -1;
            return false;
        }
...
int JMP3::GetID3TagSize(char* fname) {
    ...
    if (!strncmp((char*)header, "ea3", 3) || !strncmp((char*)header, "EA3", 3) || !strncmp((char*)header, "ID3", 3)) {
...
#ifdef MP3_SUPPORT
    if (m_channel >= 0) sceAudioSRCChRelease();
...
bool JMP3::play() { return (m_paused = false); }
...
int JMP3::playTime() const { return m_playTime; }

int JMP3::playTimeMinutes() { return (m_playTime / 1000) / 60; }

int JMP3::playTimeSeconds() { return (m_playTime / 1000) % 60; }
