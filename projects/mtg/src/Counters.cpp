...
Counter::Counter(MTGCardInstance* _target, int _power, int _toughness) { init(_target, "", _power, _toughness); }
...
int Counter::cancelCounter(int power, int toughness) {
    while (this->target->counters->hasCounter(power, toughness) &&
           this->target->counters->hasCounter(power * -1, toughness * -1)) {
...
int Counters::addCounter(const char* _name, int _power, int _toughness) {
    /*420.5n If a permanent has both a +1/+1 counter and a -1/-1 counter on it, N +1/+1 and N -1/-1 counters are
     * removed from it, where N is the smaller of the number of +1/+1 and -1/-1 counters on it.*/
...
int Counters::addCounter(int _power, int _toughness) { return addCounter("", _power, _toughness); }
...
int Counters::removeCounter(const char* _name, int _power, int _toughness) {
    for (int i = 0; i < mCount; i++) {
        if (counters[i]->sameAs(_name, _power, _toughness)) {
            ...
            dynamic_cast<WEventCounters*>(e)->targetCard = this->target;
            g->receiveEvent(e);
            // special case:if a card is suspended and no longer has a time counter when the last is removed, the card
            // is cast.
...
int Counters::removeCounter(int _power, int _toughness) { return removeCounter("", _power, _toughness); }
